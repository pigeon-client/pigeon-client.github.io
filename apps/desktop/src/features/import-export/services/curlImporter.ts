...
function normalizeHeaderEntries(headers: JSONOutput["headers"] = {}): Header[] {
...
function parseCookies(parsed: JSONOutput, headers: Header[]): Cookie[] {
  if (parsed.cookies) {
...
function parseAuth(parsed: JSONOutput, headers: Header[]): RequestModel["auth"] {
  if (parsed.auth) {
    return {
      type: "basic",
...
function hasContentType(headers: Header[], value: string): boolean {
...
function parseBody(parsed: JSONOutput, headers: Header[]): Body {
