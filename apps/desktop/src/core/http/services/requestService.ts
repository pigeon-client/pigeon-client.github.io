...
const httpClient = selectImpl({ tauri: tauriHttpClient, browser: browserHttpClient });

/** Exact text of the Rust-side cancel error (`CANCELLED_ERR` in `src-tauri/src/http.rs`). */
const CANCELLED_MESSAGE = "Request cancelled";

/** True for a user-initiated cancel (Rust cancel error, or a browser `fetch` abort). */
function isCancelledError(e: unknown): boolean {
  const name = (e as { name?: string } | undefined)?.name;
  if (name === "AbortError") return true;
  return String(e).includes(CANCELLED_MESSAGE);
}
...
export function resolveRequest(config: RequestConfig, resolve: Resolver): ResolvedRequest {
  const { sub, missing } = createAccumulatingResolver(resolve);
