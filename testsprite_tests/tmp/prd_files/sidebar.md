...
(`Sidebar.tsx`), not a feature module — still a primary UX surface.
...
`⌘F` focus header search · `⌘N` new request (global) · Import via control (no dedicated chord).
...
- Playwright: `e2e/smoke.spec.ts` plus feature specs using sidebar testids.
