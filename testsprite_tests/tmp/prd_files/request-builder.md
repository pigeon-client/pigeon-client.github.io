...
- `⌘↵` send · `⌘S` save to collection · `⌘F` focus header search
...
- Gap to watch: no dedicated E2E yet for long-URL / KV horizontal scroll (manual + `feature-qa`).
...
- Header key suggestion list has no max-height scroll if catalog grows large.
- **Known bug (2026-07-26, open):** the URL bar's `{{token}}` hover-preview is broken — the
  transparent, absolutely-positioned `url-input` sits above the syntax overlay and intercepts all
  pointer events, so `env-token` spans never receive `mouseenter`. Regressed by the long-URL
  scroll-sync rework (commit `9af2269`, "Enhance tab functionality and add SSE support"), which
  moved the input to `absolute inset-0 z-[var(--z-raised)]` over the overlay instead of the
  reverse. Full detail + repro in `docs/features/environments.md` Open risks and
  `docs/qa/2026-07-26-report.md` bug #1.
- Long-URL / long-value overflow historically regressed; treat as mandatory QA suite.
