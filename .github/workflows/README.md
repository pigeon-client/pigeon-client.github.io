...
Deploys **https://trypigeon.dev** automatically:
...
| Secret | Purpose |
|--------|---------|
| `CLOUDFLARE_API_TOKEN` | Wrangler auth — needs **Account → Workers R2 Storage → Edit** and **Account → Workers Scripts → Edit** |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare account ID (dashboard → Workers → right sidebar) |

Create the API token in Cloudflare → **My Profile → API Tokens → Create Token**. Use the
**Edit Cloudflare Workers** template and ensure R2 permissions are included, or build a custom
token with:

- Account / Workers R2 Storage / Edit
- Account / Workers Scripts / Edit
- Account / Account Settings / Read (Wrangler needs account lookup)
...
See [`docs/release.md`](../docs/release.md) for signing secrets and notarization.
